...
import { Image, ChevronDown, TestTube, Wallet, ArrowLeft } from 'lucide-react'
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg">Photos · Web3 Storage</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedAccount ? (
              <div className="relative">
                <Button
                  data-testid="photos-account-button"
                  variant="outline"
                  size="sm"
                  onClick={() => setShowAccountPicker(!showAccountPicker)}
                  className="flex items-center gap-2"
                >
                  {walletMode === 'dev' && <TestTube className="h-3 w-3 text-yellow-500" />}
                  <span data-testid="photos-account-name">
                ...
                {showAccountPicker && (
                  <div className="absolute right-0 mt-2 w-72 bg-gray-800 border border-gray-700 rounded-lg shadow-xl overflow-hidden z-50">
                    ...
                    <div className="p-2 border-b border-gray-700">
                      ...
                      <div className="text-xs text-gray-500 truncate">{selectedAccount.address}</div>
                    </div>
                    ...
                    {accounts.length > 1 && (
                      <div className="max-h-48 overflow-y-auto">
                        ...
                        {accounts.map((account) => (
                          <button
                            key={account.address}
                            data-testid={`photos-account-select-${account.name || account.address}`}
                            onClick={() => handleAccountSelect(account.address)}
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-700 ${
                              account.address === selectedAccount.address ? 'bg-gray-700/50' : ''
                            }`}
                          >
              ...
              <div className="relative">
                <Button
                  data-testid="photos-connect-button"
                  size="sm"
                ...
                {showConnectOptions && (
                  <div className="absolute right-0 mt-2 w-64 bg-gray-800 border border-gray-700 rounded-lg shadow-xl overflow-hidden z-50">
                    ...
                    <button
                      data-testid="photos-connect-dev"
                    ...
                    <button
                      data-testid="photos-connect-wallet"
