...
- **Integrity anchor (client-computed)**: the drive's metadata root is a *deterministic*
  ...
  (`provider-node/src/fs_index.rs`), where each file's `data_root` is the content root the client
...
an append-only MMR — `provider-node/src/storage/in_memory.rs`). You never edit bytes in place; a
...
`provider-node/src/fs_index.rs`) → `setRoot(rootCid)`. Verify: re-`ls`, byte-compare a downloaded
