...
// the provider's path-based API. Plain `fetch`, no auth headers: the local dev
// provider runs `/fs` auth disabled (the M1 Writer grant covers the auth-enabled
// case, exercised by the browser in M6). Reused unchanged by the UI later.
...
function fsBase(providerUrl: string, bucketId: bigint): string {
  return `${providerUrl}/fs/${Number(bucketId)}`;
}
...
export async function mkdir(providerUrl: string, bucketId: bigint, path: string): Promise<void> {
  const res = await fetch(`${fsBase(providerUrl, bucketId)}/mkdir?path=${encodeURIComponent(path)}`, { method: "POST" });
...
export async function putFile(
  ...
  const res = await fetch(`${fsBase(providerUrl, bucketId)}/file?path=${encodeURIComponent(path)}`, {
    method: "PUT",
...
export async function listDir(providerUrl: string, bucketId: bigint, path: string, recursive = false): Promise<LsEntry[]> {
  ...
  const res = await fetch(`${fsBase(providerUrl, bucketId)}/ls?${params.toString()}`);
...
export async function indexRoot(providerUrl: string, bucketId: bigint): Promise<IndexRoot> {
  const res = await fetch(`${fsBase(providerUrl, bucketId)}/index_root`);
...
export async function enumerateEntries(providerUrl: string, bucketId: bigint): Promise<MerkleEntry[]> {
  ...
  for (const e of listing) {
    if (e.entryType === "file") {
      ...
      entries.push({ path: e.path, dataRoot: computeDataRoot(bytes), size: BigInt(e.size) });
    } else {
      entries.push({ path: e.path, dataRoot: new Uint8Array(32), size: 0n });
    }
  }
  return entries;
}
