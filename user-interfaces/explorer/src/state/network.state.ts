...
import { resetSnapshot } from '@/state/explorer.state'
...
export async function selectNetwork(id: NetworkId): Promise<void> {
  ...
  // Reconnect to new endpoint; the old network's snapshot must not survive
  // the switch (nor be published late by a load that was already in flight).
  disconnect()
  resetSnapshot()
...
export async function selectCustomNetwork(input: {
  ...
  // Reconnect to new endpoint (see selectNetwork on why the snapshot resets)
  disconnect()
  resetSnapshot()
  await connect(config.parachainWs)
}
