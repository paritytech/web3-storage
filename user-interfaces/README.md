...
- **Tier 2 — `integration-tests.yml` (`ui-integration-tests` job)**: full Playwright e2e against zombienet + provider, reusing the shared `build` artifact instead of rebuilding. Runs on every PR (like the rest of the integration suite) and is gated by the required **Integration Tests** check. **Required from day 1** — robustness comes from idempotent test setup, generous CI timeouts, and detect-and-skip for state collisions, not from softening the gate.
