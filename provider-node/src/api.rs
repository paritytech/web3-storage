...
/// - `chain_state_not_ready` — `current_relay_block` and `request_timeout` are
...
async fn negotiate_terms(
    ...
    // Both the relay block and RequestTimeout must be known before we can sign —
    // otherwise we'd emit unbounded or already-expired terms.
    let relay_block = state
        .chain_state
        .current_relay_block
    ...
    let request_timeout = state
        ...
        .unwrap_or(0);
    if relay_block == 0 || request_timeout == 0 {
    ...
    let terms: AgreementTermsOf = AgreementTerms {
        ...
        valid_until: relay_block.saturating_add(request_timeout),
