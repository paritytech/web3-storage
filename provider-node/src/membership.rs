...
//! Chain-backed bucket membership resolution.
//!
//! Implements `provider-auth`'s [`MembershipResolver`] seam against the node's
//! shared subxt connection. Lives here rather than in `provider-auth` so that
//! crate stays free of a chain dependency for client-side consumers.

use crate::chain_connection::{self, ChainWatch};
use provider_auth::MembershipResolver;
...
impl ChainMembershipResolver {
    ...
    /// Resolved per lookup, not once up front, so a reconnect is picked up by
    /// the next request. Failing before the first connect is expected and
    /// retryable: it surfaces to the caller as a lookup error.
