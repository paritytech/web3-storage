...
impl DiskStorage {
    ...
    ///
    /// The cited commitment covers exactly `commitment_leaf_count` leaves from
    /// `commitment_start_seq`. Later commits grow the MMR (different root) and
    /// prunes shift the window, so the proof is generated by rebuilding that
    /// exact prefix of the leaf history and proving `leaf_index` inside it.
