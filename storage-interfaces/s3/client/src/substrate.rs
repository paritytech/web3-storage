...
impl SubstrateClient {
	...
	fn signer(&self) -> std::result::Result<&Keypair, String> {
		self.signer.as_ref().map(|s| s.as_ref()).ok_or_else(|| "No signer configured".to_string())
	}
	...
	pub async fn create_s3_bucket(
		...
		debug!("Creating S3 bucket: {} (min_providers={})", name, min_providers);
	...
	pub async fn put_object_metadata(
		...
		let metadata_values: Vec<Value> = user_metadata
			.into_iter()
			.map(|(k, v)| {
				Value::unnamed_composite([Value::from_bytes(&k), Value::from_bytes(&v)])
			})
			.collect();
	...
	pub async fn delete_object_metadata(
		...
		debug!("Deleting object metadata: bucket={}, key={}", bucket_id, key);
	...
	pub async fn get_object_metadata(
		...
		match result {
			Some(value) => {
				...
				let cid =
					if cid_bytes.len() == 32 { H256::from_slice(&cid_bytes) } else { H256::zero() };

				let size = extract_u64_field(&decoded, "size").unwrap_or(0);
				let last_modified = extract_u64_field(&decoded, "last_modified").unwrap_or(0);
				let content_type = extract_bytes_field(&decoded, "content_type").unwrap_or_default();
	...
	pub async fn list_objects(
		...
		let bucket_info = self.get_bucket_info(bucket_id).await?.ok_or("Bucket not found")?;
