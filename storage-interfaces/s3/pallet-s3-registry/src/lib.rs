...
pub mod pallet {
	...
	pub type Objects<T: Config> =
		StorageDoubleMap<_, Blake2_128Concat, S3BucketId, Blake2_128Concat, ObjectKey, ObjectMetadata, OptionQuery>;
	...
	pub enum Event<T: Config> {
		...
		ObjectPut { s3_bucket_id: S3BucketId, key: Vec<u8>, cid: H256, size: u64 },
		/// Object deleted.
		ObjectDeleted { s3_bucket_id: S3BucketId, key: Vec<u8> },
		/// Object copied.
		ObjectCopied {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_s3_bucket(
			...
			let bounded_name: BucketName =
				name.clone().try_into().map_err(|_| Error::<T>::InvalidBucketName)?;

			// Check name uniqueness
			ensure!(!BucketNameToId::<T>::contains_key(&bounded_name), Error::<T>::BucketNameExists);
		...
		pub fn put_object_metadata(
			...
			let bounded_key: ObjectKey =
				key.clone().try_into().map_err(|_| Error::<T>::ObjectKeyTooLong)?;

			// Convert content type
			let bounded_content_type: BoundedVec<u8, MaxContentTypeLen> =
				content_type.try_into().map_err(|_| Error::<T>::ContentTypeTooLong)?;
			...
			Self::deposit_event(Event::ObjectPut { s3_bucket_id, key, cid, size });
		...
		pub fn copy_object_metadata(
			...
			// Convert keys
			let bounded_src_key: ObjectKey =
				src_key.clone().try_into().map_err(|_| Error::<T>::ObjectKeyTooLong)?;
			let bounded_dst_key: ObjectKey =
				dst_key.clone().try_into().map_err(|_| Error::<T>::ObjectKeyTooLong)?;
