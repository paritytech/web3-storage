...
fn register_provider(who: u64) {
	...
	let public_key: BoundedVec<u8, frame_support::traits::ConstU32<64>> =
		b"01234567890123456789012345678901".to_vec().try_into().unwrap();
...
fn put_object_metadata_works() {
	new_test_ext().execute_with(|| {
		...
		let key: BoundedVec<u8, MaxObjectKeyLen> =
			b"folder/test.txt".to_vec().try_into().unwrap();
...
fn delete_object_metadata_works() {
	new_test_ext().execute_with(|| {
		...
		let key: BoundedVec<u8, MaxObjectKeyLen> =
			b"test.txt".to_vec().try_into().unwrap();
...
fn copy_object_metadata_works() {
	new_test_ext().execute_with(|| {
		...
		let key: BoundedVec<u8, MaxObjectKeyLen> =
			b"copy.txt".to_vec().try_into().unwrap();
