...
#[test]
fn deregister_provider_and_returns_stake() {
    new_test_ext().execute_with(|| {
        let multiaddr = b"/ip4/127.0.0.1/tcp/3000".to_vec();
        assert_ok!(StorageProvider::register_provider(
            RuntimeOrigin::signed(1),
        ...
        // Single call — no announcement window.
...
fn withdraw_agreement_request_works_after_provider_deregisters() {
    new_test_ext().execute_with(|| {
        ...
        // Set up an agreement, then end it so committed_bytes drops to 0.
        ...
        // committed_bytes is now 0 — deregister succeeds.
        assert_ok!(StorageProvider::deregister_provider(RuntimeOrigin::signed(
            2
        )));
        assert!(Providers::<Test>::get(2).is_none());

        // The agreement was already removed when claimed; further end_agreement
        // calls with the deregistered provider return AgreementNotFound, not a panic.
        assert_noop!(
...
fn deregister_provider_full_flow() {
    new_test_ext().execute_with(|| {
        // register → agreement → end → deregister → re-register
        register_provider(2, 200);

        // Open an agreement (max_bytes 50 fits within stake 200).
        let bucket_id = setup_agreement(2, 1, 50, 100);

        // Provider has active commitment — deregister is gated.
        assert_noop!(
            StorageProvider::deregister_provider(RuntimeOrigin::signed(2)),
            Error::<Test>::ProviderHasActiveAgreements
        );

        // Early-terminate via Pay so committed_bytes drops back to 0.
        ...
        // Now deregister succeeds in a single call.
        assert_ok!(StorageProvider::deregister_provider(RuntimeOrigin::signed(
            2
        )));
        assert!(Providers::<Test>::get(2).is_none());

        // Re-registration works — the record was fully removed, not flagged.
        register_provider(2, 200);
        assert!(Providers::<Test>::get(2).is_some());
    });
}
