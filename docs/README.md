...
- **[Potential RFC: Bucket Lifecycle and Transfer Between Providers](./drafts/RFC_BUCKET_TRANSFER.md)** — bucket states and transitions as implemented, what is missing, and the transfer story: a dApp bucket whose data was written by other users must continue on a new primary provider with the same `bucket_id` and without anyone re-uploading. Main questions, gaps and drifts for the design owner. No proposal yet.
