...
- **[Bucket Transfer Between Providers: Design Gaps](./drafts/RFC_BUCKET_TRANSFER.md)** — a dApp bucket whose data was written by other users must continue on a new primary provider with the same `bucket_id` and without anyone re-uploading: story, designed vs. implemented, gaps, drifts, and questions for the design owner. No proposal.
