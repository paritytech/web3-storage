...
- **[RFC: Bucket Transfer Between Providers](./drafts/RFC_BUCKET_TRANSFER.md)** — a dApp bucket whose data was written by other users must continue on a new primary provider without anyone re-uploading: designed vs. implemented, proposed `add_primary_agreement` + role-independent catch-up sync + provider-origin attestation, continuity and retention gaps.
