...
- **[Potential RFC: Bucket Transfer Between Providers](./drafts/RFC_BUCKET_TRANSFER.md)** — a dApp bucket whose data was written by other users must continue on a new primary provider with the same `bucket_id` and without anyone re-uploading: story, main questions, gaps and drifts for the design owner. No proposal yet.
